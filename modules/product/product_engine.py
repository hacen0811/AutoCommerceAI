...
class ProductEngine:
    ...
    안전 원칙:
    - 리뷰 수집 실패 시 기존 파이프라인은 계속 실행
    - 기존 반환 구조 유지
    - Sprint66 리뷰 필드만 추가
    """

    ENGINE_VERSION = "product-engine-2.1-sprint66"
    ...
    def build_from_coupang(
        ...
        review_source = review_result.get(
            "source",
            "coupang_page",
        )

        review_collect_status = review_result.get(
            "status",
            "empty",
        )
        ...
        project_payload = {
            ...
            "category": resolved_category,
            "keyword": (
                keyword_data.get("comment_keyword")
                or "정보"
            ),
    ...
    def _collect_reviews(
        ...
        """
        리뷰 수집 오류가 ProductEngine 전체 실행을
        중단시키지 않도록 안전하게 처리한다.
        """
