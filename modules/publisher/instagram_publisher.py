...
class InstagramPublisher:
    """
    Sprint82-3 Instagram Reels Publisher

    역할:
    - Sprint82-1 PublisherEngine의 instagram_reels 게시 준비 데이터를 입력으로 받음
    - 실제 Instagram API 호출 없이 업로드 직전 페이로드를 생성
    - 캡션, 해시태그, CTA, 예약 시간, 제휴 고지 문구를 검증
    - 원본 입력을 변경하지 않고 독립적으로 동작
    """

    VERSION = "instagram-publisher-82-3"
    ...
    def build(
        ...
        resolved_cover_path = self._clean_text(cover_image_path)
        ...
        checks = {
            ...
            "visibility_valid": resolved_visibility in {"private", "public"},
        ...
        payload = {
            ...
            "publish_settings": {
                ...
                "cover_image_path": resolved_cover_path,
            },
            "content": {
                "script": script,
                "cta": cta,
        ...
        print("[Sprint82-3 Instagram] Version:", self.VERSION, flush=True)
        print(
            "[Sprint82-3 Instagram] Source:",
            result["source_version"],
            flush=True,
        )
        print(
            "[Sprint82-3 Instagram] Ready:",
            result["upload_ready"],
            flush=True,
        )
        print(
            "[Sprint82-3 Instagram] Title:",
            payload["title"],
            flush=True,
        )
        print(
            "[Sprint82-3 Instagram] Video:",
            payload["video_path"],
            flush=True,
        )
    ...
    def _validate_source(
        ...
        if not platform_payload:
            errors.append("instagram_reels 게시 데이터가 없습니다")
            return {"valid": False, "errors": errors, "warnings": warnings}

        if not bool(platform_payload.get("ready")):
            errors.append("instagram_reels 게시 데이터가 준비 상태가 아닙니다")
        ...
        if not self._normalize_hashtags(platform_payload.get("hashtags")):
            warnings.append("Instagram 해시태그가 비어 있습니다")
