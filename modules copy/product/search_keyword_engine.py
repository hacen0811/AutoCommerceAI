...
class SearchKeywordEngine:
    ...
    def detect_category(self, clean, tokens, category=""):
        ...
        # Sprint 53
        # 특정 상품 핵심명사가 강한 경우,
        # "수납/정리/선반" 같은 일반 단어 때문에 kitchen_storage로 오판정되는 것을 방지합니다.
        generic_storage_words = ["수납", "수납형", "정리", "선반", "거치대", "홀더", "걸이", "랙"]
        strong_non_kitchen_words = [
            "드라이기",
            "드라이어",
            "헤어드라이기",
            "헤어드라이어",
        ...
        has_generic_storage = any(word in text for word in generic_storage_words)
        has_strong_non_kitchen = any(word in text for word in strong_non_kitchen_words)

        if has_generic_storage and has_strong_non_kitchen:
            return self.fallback_category(clean, tokens)
