...
def show_shorts_studio():
    st.title("🎞️ 쇼츠 스튜디오 4.0")
    st.caption("쿠팡 링크 + 상품명 + 원본 영상으로 쇼핑쇼츠 제작 패키지를 생성합니다.")
    st.info(
        "DB가 없어도 여기서 새 프로젝트를 만들고, "
        "검색어/영상후보/대본/CapCut/업로드 문구까지 한 번에 생성할 수 있습니다."
    )
    st.caption("4.0: 도우인/타오바오/1688 후보는 새 창 검색 중심으로 사용합니다.")
    ...
    product_name = st.text_area(
        "상품명",
        height=110,
        placeholder="쿠팡 상품명 전체를 붙여넣어 주세요.",
    )
    ...
    run_one_click = st.checkbox(
        "쇼핑쇼츠 패키지 생성 후 원클릭 파이프라인까지 실행",
        value=True,
    )
    ...
    live_collect = st.checkbox(
        "실제 웹 수집 시도(도우인/타오바오/1688)",
        value=False,
    )

    if live_collect:
        st.warning(
            "실제 웹 수집은 사이트 로그인/차단 상태에 따라 결과가 비어 있을 수 있습니다. "
            "처음에는 브라우저가 열릴 수 있습니다."
        )
    ...
    if st.button("쇼핑쇼츠 패키지 생성", use_container_width=True):
        ...
        with st.spinner("쇼핑쇼츠 패키지와 원클릭 결과를 생성 중입니다..."):
            result = StudioPipelineEngine().run(
                coupang_url,
                product_name,
                uploaded_video,
                price,
        ...
        if package.get("ai_applied"):
            ...
            st.warning(
                f"AI API 미연결 또는 실패: "
                f"{ai_status.get('message') or ai_status.get('mode')}"
            )
        ...
        with tabs[0]:
            ...
            for item in product.get("taobao_top10", []):
                st.write(
                    f"{item.get('rank')}. {item.get('query')} — "
                    f"{item.get('purpose')} / {item.get('score')}"
                )
            ...
            st.subheader("검색 바로가기")

            taobao_keyword = product.get("taobao_keyword", "")
            douyin_keyword = product.get("douyin_keyword", "")
            keyword_1688 = product.get("taobao_keyword", "") or product.get("douyin_keyword", "")
            ...
            with c3:
                st.link_button(
                    "🏭 1688",
                    make_search_url("1688", keyword_1688),
                    use_container_width=True,
                )
        ...
        with tabs[1]:
            ...
            for item in live.get("results", [])[:10]:
                ...
                url = item.get("url")
                if url:
                    st.link_button("수집 결과 새 창으로 열기", url)
            ...
            for item in sources.get("best_candidates", []):
                st.markdown(
                    f"### {item.get('rank')}. "
                    f"{item.get('platform')} / {item.get('score')}점"
                )
                st.write(item.get("title"))
                ...
                if search_url:
                    st.link_button("후보 검색 새 창으로 열기", search_url)
            ...
            for item in sources.get("candidates", []):
                st.write(
                    f"{item.get('rank')}. [{item.get('platform')}] "
                    f"{item.get('title')} — "
                    f"{item.get('purpose')} / {item.get('score')}"
                )
        ...
        with tabs[2]:
            ...
            for label in ["15s", "25s", "35s"]:
                st.subheader(f"{label} 대본")
                for item in scripts.get(label, []):
        ...
        with tabs[7]:
            ...
            copybox(
                "Vision JSON",
                json.dumps(vision, ensure_ascii=False, indent=2, default=str),
                420,
            )
        ...
        with tabs[9]:
            ...
            st.write(
                "자동 분석:",
                "성공" if coupang.get("auto_fetched") else "입력값/URL 기반",
            )
            ...
            copybox(
                "쿠팡 분석 JSON",
                json.dumps(coupang, ensure_ascii=False, indent=2, default=str),
                420,
            )
